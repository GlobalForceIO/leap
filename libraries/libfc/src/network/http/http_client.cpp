...
public:
   ...
   std::string get_sync(const url& dest, const fc::time_point& _deadline) {
      ...
      if (res.result() == http::status::internal_server_error) {
         ...
         try {
            fc::variant errors;
            errors = json::from_string(res.body());
            auto err_var = errors.get_object()["error"].get_object();
...
std::string http_client::get_sync(const url& dest, const fc::time_point& deadline) {
   if(dest.proto() == "unix")
      return _my->get_sync(_my->get_unix_url(*dest.host()), deadline);
   else
      return _my->get_sync(dest, deadline);
}

std::vector<uint8_t> http_client::get_sync_raw(const url& dest, const time_point& deadline) {
   std::string res;
   res = _my->get_sync(dest, deadline);
   std::vector<uint8_t> vec(res.begin(), res.end());
   return vec;
}
variant http_client::get_sync_json(const url& dest, const time_point& deadline) {
    return json::from_string(_my->get_sync(dest, deadline));
}
